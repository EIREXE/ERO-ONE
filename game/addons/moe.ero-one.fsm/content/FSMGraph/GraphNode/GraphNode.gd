...
#		ownerFsmRef.get_ref().get_transition(get_name()).devLog = str(leftBtn);
...
#		ownerFsmRef.get_ref().get_transition(get_name()).devLog ="creating arrow in " + get_name() + " target: " + inTagetGraphNode.get_parent().get_name();
