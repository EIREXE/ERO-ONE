...
func update_marker_transform(marker_data):
	...
	if marker.important:
		position.x = clamp(map_icon.rect_position.x, -map_icon.rect_size.x * 0.25, map_icons_container.rect_size.x - map_icon.rect_size.x * 0.75)
		position.y = clamp(map_icon.rect_position.y, -map_icon.rect_size.y * 0.25, map_icons_container.rect_size.y - map_icon.rect_size.y * 0.75)
		map_icon.rect_position = position
